...
public class Spawnengine
{
	...
	public Location findSafeSpawn(Location base, PVPPlayer player)
	{
		...
		double maxLOScomputationDistance = Main.gameEngine.configuration
				.maxLOScomputationDistance(this.match.getWorld());
			...
			for (PVPPlayer p : this.match.getPlayers())
			{
				...
				if (p.getTeam() != player.getTeam() && p.isSpawned())
				{
					...
					if (Main.gameEngine.configuration.isMinEnemySpawnDistance2D(this.match.getWorld()))
						vect.setY(0d);
					Area3D dzone = new Area3D(p.thePlayer.getLocation().clone().add(vect),
							p.thePlayer.getLocation().clone().add(vect.clone().multiply(-1d)));
					if (dzone.isCoordInsideRegion(current))
					{
						safe = false;
						Debugger.writeDebugOut(
								String.format("Spawn not safe for '%s' due to near enemy player '%s' Radius: %.2f",
										player.thePlayer.getName(), p.thePlayer.getName(), radius));
						break;
					}
					if (safe && radius <= maxLOScomputationDistance)
					{
						...
						{
							safe = false;
							Debugger.writeDebugOut(String.format(
									"Spawn not safe for '%s' due to look from enemy player '%s' Radius: %.2f Look-angle: %.2f°",
									player.thePlayer.getName(), p.thePlayer.getName(), radius, lookAngle));
							break;
						}
					}
					if (safe)
