...
public class RadioStation
{
	...
	public void doUpdate()
	{
		if (timer > 0.1d * GameEngine.tps)
		{
			...
			sign.setLine(1, (destroyed ? ChatColor.DARK_RED + "DESTROYED"
					: (armed ? ChatColor.RED + "ARMED" : ChatColor.DARK_GREEN + "RUNNING")));
			sign.setLine(2, (destructTime == 0d ? "INSTANT"
					: Integer.toString((int) Math.round(destructTimer / destructTime * 100d))));
			sign.setLine(3, Integer.toString(match.getRemainingStations()) + " | "
					+ Integer.toString((int) Math.round(match.getTeamRed().getPoints())));
			sign.update(true);
		}
		timer++;
	}
