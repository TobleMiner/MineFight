package tobleminer.minefight.engine.player.resupply;
...
public class ResupplyStation
{
	...
	public ResupplyStation(Sign sign, Match match, PVPPlayer owner, double ammoRefillDist, double ammoRefillSpeed,
			int amount)
	{
		this.sign = sign;
		this.match = match;
		...
		if (timer > GameEngine.tps / 2d) // Update each 0.5s
		{
			try
			{
				if (time > ammoRefillSpeed)
				{
					...
					else if (!(woolRedBack.getLocation().getBlock().getType().equals(Material.WOOL)
							&& woolRedBack.getLocation().getBlock().getData() == (byte) 14))
					{
						broken = true;
					}
					else if (!(woolRedLeft.getLocation().getBlock().getType().equals(Material.WOOL)
							&& woolRedLeft.getLocation().getBlock().getData() == (byte) 14))
					{
						broken = true;
					}
					else if (!(woolRedRight.getLocation().getBlock().getType().equals(Material.WOOL)
							&& woolRedRight.getLocation().getBlock().getData() == (byte) 14))
					{
						broken = true;
					}
					else if (!(woolWhiteBackUp.getLocation().getBlock().getType().equals(Material.WOOL)
							&& woolWhiteBackUp.getLocation().getBlock().getData() == (byte) 0))
					{
						broken = true;
					}
					if (broken)
						...
						if (dist <= ammoRefillDist)
						{
							if (player.isSpawned())
							{
								...
								if (cc != null)
								{
									...
									{
										if (!(pi.contains(is.clone()) || (is.getType() == Material.WOOL
												|| is.getType() == Material.SIGN || is.getType() == Material.CHEST)))
										{
											fill--;
											...
											if (pi.contains(is.getType()))
											{
												...
												{
													pi.addItem(is);
													EffectSyncCalls.playSound(player.thePlayer.getLocation(),
															Sound.BAT_TAKEOFF, 4f, 0f);
												}
											}
											else
											{
