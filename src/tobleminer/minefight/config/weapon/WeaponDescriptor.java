...
public class WeaponDescriptor
{
	...
	public WeaponDescriptor(String name, boolean doTranslate, double cadence, double speed, String useType,
			String dmgType, Material material, short itemdmg, List<Entry<Double, Double>> damage,
			List<Entry<HitZone, Double>> multipliers, Material ammomat, int firemode)
	{
		this.name = name;
	...
	public double getDamage(double length)
	{
		...
		{
			if ((above == null && entry.getKey() > length)
					|| (above != null && entry.getKey() > length && entry.getKey() < above.getKey()))
			{
				above = entry;
				continue;
			}
			if ((under == null && entry.getKey() < length)
					|| (under != null && entry.getKey() < length && entry.getKey() > under.getKey()))
			{
				under = entry;
				continue;
			}
	...
	{
		HIT("hit"), FLAMETHROWER("flamethrower", "flame"), MEDIGUN("medigun", "medi"), PROJECTILEHIT("projectilehit",
				"projhit", "phit"), NONE("applejack");
	...
	{
		HIT("hit"), BLOCK("block", "blocking"), NONE("fluttershy");
