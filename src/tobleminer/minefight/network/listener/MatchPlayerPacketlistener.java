package tobleminer.minefight.network.listener;
...
public class MatchPlayerPacketlistener extends PacketAdapter
{
	...
	public void onPacketSending(PacketEvent pe)
	{
		...
		// ID:"+pc.getID()+"vs"+Packets.Server.NAMED_ENTITY_SPAWN);
		...
		/*
		 * else if(pc.getID() == Packets.Server.ITEM_DATA) { String name =
		 * "#YOLO"; char[] schar = name.toCharArray(); byte[] data = new
		 * byte[schar.length]; for(int i=0;i<schar.length;i++) { data[i] =
		 * (byte)schar[i]; } pc.getSpecificModifier(byte[].class).write(0,data);
		 * }
		 */
	}
